...
function SignUpForm() {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (signUpError) {
      ...
      setError(getErrorMessage(signUpError) || 'Falha ao criar conta');
      setLoading(false);
      return;
    }
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          >
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex items-center justify-center gap-2 rounded-lg bg-[#2563EB] py-3 text-sm font-semibold text-white transition-all hover:bg-[#1D4ED8] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed mt-1"
            >
              {loading ? (
                <>
                  <div
            ...
            <div className="text-center pt-2">
              ...
              >
                Já tem conta? <span className="text-[#2563EB] font-medium">Entrar</span>
              </a>
            </div>
          </form>
        </div>
